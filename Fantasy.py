...
from fantasy_events_data import (event_categories, locations, factions, characters, 
                               magic_fields, resources, monsters, other_realms, 
                               inn_names, fill_ins)
...
class FantasyWorldEventGenerator:
    ...
    def process_and_enhance_event(self, event_text: str, category: str) -> Dict[str, Any]:
        ...
        if self.gemini_available:
            ...
            if 'visual_description' in ai_details and ai_details['visual_description']:
                image_path = self.ai.generate_event_image(ai_details['visual_description'], self.event_count, self.images_dir)
                if image_path:
            ...
            event_data['headline'] = news_summary['headline']
            event_data['description'] = news_summary['description']
            ...
            print("-"*40)
            print(f"{cyan}{news_summary['headline']}{reset}")
            print(f"{green}{news_summary['description']}{reset}")
            print("="*40 + "\n")
        ...
        if self.telegram.get_chat_id():
            # Use the news style summary if available, otherwise create a simpler version
            if 'headline' in event_data and 'description' in event_data:
                telegram_message = news_summary['formatted_message'] 
            else:
                # Create a summarized version of the event
                news_summary = self.ai.summarize_event_for_telegram(event_text, category, self.world_state, self.world_name)
                telegram_message = news_summary['formatted_message']
    ...
    def apply_random_world_changes(self):
        ...
        change_type = random.choice([
            ...
            'character_development', 'realm_shift'
        ])
